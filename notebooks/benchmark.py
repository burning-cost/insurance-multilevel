"""
Benchmark: TwoStageMultilevel vs plain CatBoost
================================================

Demonstrates the core value proposition of insurance-multilevel:
better predictions for thin groups via Bühlmann-Straub credibility shrinkage.

The synthetic portfolio has ~50 brokers with very different observation counts
(small <20, medium 20-100, large >100). Each broker has a true random effect
drawn from N(0, tau2=0.10). The question is: which model recovers broker-level
accuracy more faithfully — especially for the thin brokers where you can't trust
raw experience?

Expected results:
- MultilevelPricingModel should beat plain CatBoost for small and medium groups
- For large groups (>100 obs), both should be close because there's enough data
  for CatBoost to partially pick up group patterns indirectly
- The A/E ratio by group size should be more stable for the multilevel model

Run on Databricks (requires catboost, polars, scipy).
"""

import time
import warnings
from typing import Any

import numpy as np
import polars as pl

warnings.filterwarnings("ignore")


# ---------------------------------------------------------------------------
# Data generation
# ---------------------------------------------------------------------------

def generate_synthetic_portfolio(
    n_policies: int = 8_000,
    n_brokers: int = 50,
    tau2: float = 0.10,    # between-broker variance
    sigma2: float = 0.25,  # within-broker noise (after risk factors)
    seed: int = 42,
) -> tuple[pl.DataFrame, pl.DataFrame, dict[str, float]]:
    """
    Generate a synthetic motor portfolio with known broker random effects.

    The data generating process:
        log(y_i) = mu(x_i) + b_g(i) + eps_i

    where:
        mu(x_i)  = fixed effects from risk factors (CatBoost-learnable structure)
        b_g      ~ N(0, tau2)    broker random effect
        eps_i    ~ N(0, sigma2)  idiosyncratic noise

    Broker sizes follow a Zipf distribution to create the small/medium/large
    groups that test credibility shrinkage.

    Returns
    -------
    train_df, test_df, true_broker_effects (dict)
    """
    rng = np.random.default_rng(seed)

    # Broker sizes: Zipf-like distribution (one large, many small)
    # This mimics a real portfolio with a few large brokers and many small ones
    raw_sizes = rng.integers(3, 400, size=n_brokers).astype(float)
    # Bias towards small brokers by squeezing the distribution
    raw_sizes = rng.exponential(scale=80, size=n_brokers).clip(3, 500).astype(int)
    total = raw_sizes.sum()
    broker_counts = np.round(raw_sizes / total * n_policies).astype(int)
    # Fix any rounding error
    diff = n_policies - broker_counts.sum()
    broker_counts[0] += diff
    broker_counts = np.clip(broker_counts, 1, None)

    broker_ids = [f"broker_{i:03d}" for i in range(n_brokers)]

    # True broker random effects
    true_effects = {bid: float(rng.normal(0, np.sqrt(tau2))) for bid in broker_ids}

    # Generate policies
    rows = []
    for bid, n in zip(broker_ids, broker_counts):
        n = int(n)
        driver_age = rng.integers(17, 80, size=n)
        vehicle_group = rng.integers(1, 50, size=n)
        ncd_years = rng.integers(0, 10, size=n)
        annual_mileage = rng.lognormal(9.0, 0.5, size=n).astype(int)

        # Fixed effect: structural log-premium (what CatBoost should learn)
        log_mu = (
            4.5                                             # base
            + 0.025 * vehicle_group                        # vehicle risk
            - 0.08 * ncd_years                             # NCD discount
            + 0.40 * (driver_age < 25).astype(float)       # young driver load
            + 0.20 * (driver_age > 70).astype(float)       # old driver load
            + 0.00015 * annual_mileage                     # mileage
        )

        # Add broker random effect + idiosyncratic noise
        b_g = true_effects[bid]
        eps = rng.normal(0, np.sqrt(sigma2), size=n)
        log_y = log_mu + b_g + eps
        y = np.exp(log_y)

        for j in range(n):
            rows.append({
                "broker_id": bid,
                "driver_age": int(driver_age[j]),
                "vehicle_group": int(vehicle_group[j]),
                "ncd_years": int(ncd_years[j]),
                "annual_mileage": int(annual_mileage[j]),
                "log_mu": float(log_mu[j]),
                "y": float(y[j]),
            })

    df = pl.DataFrame(rows)

    # 80/20 train/test split stratified by broker
    rng2 = np.random.default_rng(seed + 1)
    indices = rng2.permutation(len(df)).tolist()
    split = int(len(df) * 0.8)
    train_df = df[indices[:split]]
    test_df = df[indices[split:]]

    return train_df, test_df, true_effects


# ---------------------------------------------------------------------------
# Metrics
# ---------------------------------------------------------------------------

def rmse(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    return float(np.sqrt(np.mean((y_true - y_pred) ** 2)))


def mae(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    return float(np.mean(np.abs(y_true - y_pred)))


def gini_coefficient(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    """Normalised Gini (2*AUC - 1). Standard insurance discrimination metric."""
    n = len(y_true)
    if n == 0 or np.sum(y_true) == 0:
        return 0.0
    order = np.argsort(y_pred)[::-1]
    y_sorted = y_true[order]
    cumsum = np.cumsum(y_sorted)
    gini = (np.sum(cumsum) / np.sum(y_true) - (n + 1) / 2) / n
    return float(gini * 2)


def ae_ratio(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    """Actual/Expected ratio. Should be 1.0 for a calibrated model."""
    if np.sum(y_pred) == 0:
        return float("nan")
    return float(np.sum(y_true) / np.sum(y_pred))


def group_size_bucket(n: int) -> str:
    if n < 20:
        return "small (<20)"
    elif n <= 100:
        return "medium (20-100)"
    else:
        return "large (>100)"


# ---------------------------------------------------------------------------
# Main benchmark
# ---------------------------------------------------------------------------

def run_benchmark():
    print("=" * 65)
    print("insurance-multilevel  benchmark")
    print("TwoStageMultilevel vs plain CatBoost")
    print("=" * 65)
    print()

    # Import here so errors are visible
    from catboost import CatBoostRegressor
    from insurance_multilevel import MultilevelPricingModel, lift_from_random_effects

    print("Generating synthetic portfolio...")
    print("  50 brokers, 8,000 policies, tau2=0.10, sigma2=0.25")
    print()
    train_df, test_df, true_effects = generate_synthetic_portfolio(
        n_policies=8_000,
        n_brokers=50,
        tau2=0.10,
        sigma2=0.25,
        seed=42,
    )

    feature_cols = ["driver_age", "vehicle_group", "ncd_years", "annual_mileage"]

    y_train = train_df["y"].to_numpy()
    y_test = test_df["y"].to_numpy()

    # -----------------------------------------------------------------------
    # Baseline: plain CatBoost (no broker effects)
    # -----------------------------------------------------------------------
    print("Fitting baseline: plain CatBoost (no group effects)...")
    t0 = time.time()

    X_train_base = train_df.select(feature_cols).to_pandas()
    X_test_base = test_df.select(feature_cols).to_pandas()

    baseline_cb = CatBoostRegressor(
        iterations=300,
        learning_rate=0.05,
        depth=6,
        loss_function="RMSE",
        verbose=0,
        random_seed=42,
        allow_writing_files=False,
    )
    baseline_cb.fit(X_train_base, y_train)
    baseline_preds = baseline_cb.predict(X_test_base).astype(float)
    t_baseline = time.time() - t0
    print(f"  Done in {t_baseline:.1f}s")
    print()

    # -----------------------------------------------------------------------
    # TwoStageMultilevel
    # -----------------------------------------------------------------------
    print("Fitting MultilevelPricingModel (CatBoost + REML)...")
    t0 = time.time()

    model = MultilevelPricingModel(
        catboost_params={"iterations": 300, "learning_rate": 0.05, "depth": 6},
        random_effects=["broker_id"],
        min_group_size=5,
        reml=True,
    )
    model.fit(
        train_df.select(feature_cols + ["broker_id"]),
        train_df["y"],
        group_cols=["broker_id"],
    )
    multilevel_preds = model.predict(
        test_df.select(feature_cols + ["broker_id"]),
        group_cols=["broker_id"],
    )
    stage1_preds = model.stage1_predict(
        test_df.select(feature_cols + ["broker_id"])
    )
    t_multilevel = time.time() - t0
    print(f"  Done in {t_multilevel:.1f}s")
    print()

    # -----------------------------------------------------------------------
    # Variance components
    # -----------------------------------------------------------------------
    vc = model.variance_components["broker_id"]
    print("Variance component recovery (broker_id):")
    print(f"  True  tau2={0.10:.4f}  sigma2={0.25:.4f}  ICC={0.10/(0.10+0.25):.4f}")
    est_tau2 = vc.tau2.get("broker_id", 0.0)
    est_sigma2 = vc.sigma2
    est_icc = est_tau2 / (est_tau2 + est_sigma2) if (est_tau2 + est_sigma2) > 0 else 0.0
    print(f"  Est.  tau2={est_tau2:.4f}  sigma2={est_sigma2:.4f}  ICC={est_icc:.4f}")
    print(f"  Converged: {vc.converged}  Iterations: {vc.iterations}")
    print()

    # -----------------------------------------------------------------------
    # Overall metrics
    # -----------------------------------------------------------------------
    print("Overall test set metrics:")
    print("-" * 50)
    headers = ["Metric", "Plain CatBoost", "MultilevelModel"]
    rows_out = [
        ["RMSE",         f"{rmse(y_test, baseline_preds):.4f}",    f"{rmse(y_test, multilevel_preds):.4f}"],
        ["MAE",          f"{mae(y_test, baseline_preds):.4f}",     f"{mae(y_test, multilevel_preds):.4f}"],
        ["Gini",         f"{gini_coefficient(y_test, baseline_preds):.4f}", f"{gini_coefficient(y_test, multilevel_preds):.4f}"],
        ["A/E ratio",    f"{ae_ratio(y_test, baseline_preds):.4f}", f"{ae_ratio(y_test, multilevel_preds):.4f}"],
    ]
    col_widths = [max(len(r[i]) for r in [headers] + rows_out) for i in range(3)]
    fmt = "  ".join(f"{{:<{w}}}" for w in col_widths)
    print(fmt.format(*headers))
    print("  " + "-" * (sum(col_widths) + 4))
    for row in rows_out:
        print(fmt.format(*row))
    print()

    # -----------------------------------------------------------------------
    # Lift from random effects
    # -----------------------------------------------------------------------
    lift = lift_from_random_effects(y_test, stage1_preds, multilevel_preds)
    print("Lift from Stage 2 (random effects over CatBoost only):")
    print(f"  RMSE improvement:  {lift['rmse_improvement_pct']:.2f}%")
    print(f"  MALR improvement:  {lift['malr_improvement_pct']:.2f}%")
    print()

    # -----------------------------------------------------------------------
    # Metrics by group size bucket
    # -----------------------------------------------------------------------
    # Get broker observation counts from training data
    broker_train_counts = (
        train_df.group_by("broker_id")
        .agg(pl.len().alias("n_train"))
        .to_pandas()
        .set_index("broker_id")["n_train"]
        .to_dict()
    )

    test_with_preds = test_df.with_columns([
        pl.Series("baseline_pred", baseline_preds),
        pl.Series("multilevel_pred", multilevel_preds),
    ]).to_pandas()

    test_with_preds["n_train"] = test_with_preds["broker_id"].map(broker_train_counts).fillna(0)
    test_with_preds["size_bucket"] = test_with_preds["n_train"].apply(group_size_bucket)

    print("Metrics by group size (training count):")
    print("-" * 75)
    bucket_headers = ["Size bucket", "N policies", "RMSE baseline", "RMSE multilevel", "A/E baseline", "A/E multilevel"]
    bucket_rows = []

    for bucket in ["small (<20)", "medium (20-100)", "large (>100)"]:
        sub = test_with_preds[test_with_preds["size_bucket"] == bucket]
        if len(sub) == 0:
            continue
        yt = sub["y"].values
        yb = sub["baseline_pred"].values
        ym = sub["multilevel_pred"].values
        bucket_rows.append([
            bucket,
            str(len(sub)),
            f"{rmse(yt, yb):.4f}",
            f"{rmse(yt, ym):.4f}",
            f"{ae_ratio(yt, yb):.4f}",
            f"{ae_ratio(yt, ym):.4f}",
        ])

    col_w = [max(len(r[i]) for r in [bucket_headers] + bucket_rows) for i in range(6)]
    fmt2 = "  ".join(f"{{:<{w}}}" for w in col_w)
    print(fmt2.format(*bucket_headers))
    print("  " + "-" * (sum(col_w) + 10))
    for row in bucket_rows:
        print(fmt2.format(*row))
    print()

    # -----------------------------------------------------------------------
    # Credibility summary (top 5 and bottom 5 by n_obs)
    # -----------------------------------------------------------------------
    cred = model.credibility_summary()
    print("Credibility summary (5 largest and 5 smallest brokers):")
    top5 = cred.sort("n_obs", descending=True).head(5)
    bot5 = cred.sort("n_obs", descending=False).head(5)
    combined = pl.concat([top5, bot5])
    print(combined.select(["group", "n_obs", "credibility_weight", "multiplier", "tau2", "sigma2"]).to_pandas().to_string(index=False))
    print()

    print("=" * 65)
    print("Interpretation:")
    print("  - RMSE improvement should be positive (multilevel < baseline)")
    print("  - Small groups should show larger relative improvement")
    print("  - Credibility weights (Z) should be <0.5 for small brokers")
    print("  - A/E ratio should be closer to 1.0 for multilevel model")
    print("=" * 65)


if __name__ == "__main__":
    run_benchmark()
