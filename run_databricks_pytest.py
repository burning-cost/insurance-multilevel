"""
Submit insurance-multilevel pytest suite to Databricks serverless compute.
...
WORKSPACE_FOLDER = "/Workspace/insurance-multilevel"
NOTEBOOK_PATH = f"{WORKSPACE_FOLDER}/run_pytest"

# ---------------------------------------------------------------------------
# Read source files
# ---------------------------------------------------------------------------
BASE = "/home/ralph/repos/insurance-multilevel"
...
src_files = {
    "__init__.py":       read_file(f"{BASE}/src/insurance_multilevel/__init__.py"),
    "_types.py":         read_file(f"{BASE}/src/insurance_multilevel/_types.py"),
    "_reml.py":          read_file(f"{BASE}/src/insurance_multilevel/_reml.py"),
    "_model.py":         read_file(f"{BASE}/src/insurance_multilevel/_model.py"),
    "_diagnostics.py":   read_file(f"{BASE}/src/insurance_multilevel/_diagnostics.py"),
}

test_files = {
    "conftest.py":          read_file(f"{BASE}/tests/conftest.py"),
    "test_types.py":        read_file(f"{BASE}/tests/test_types.py"),
    "test_reml.py":         read_file(f"{BASE}/tests/test_reml.py"),
...
packages = ["src/insurance_multilevel"]
"""
...
# MAGIC %pip install polars>=0.20 numpy>=1.24 catboost>=1.2 scipy>=1.10 pytest>=7.0 hatchling --quiet
...
pkg_dir = f"/tmp/insurance_multilevel_{{pkg_id}}"
src_dir = f"{{pkg_dir}}/src/insurance_multilevel"
...
if r.returncode != 0:
    ...
    print("insurance-multilevel installed from", pkg_dir)
...
submit_body = {
    "run_name": f"insurance-multilevel-pytest-{RUN_ID}",
    "tasks": [
        {
            "task_key": "pytest",
            "notebook_task": {
